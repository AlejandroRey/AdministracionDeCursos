...
public class SalaDisponibilidadControlador implements PropertyChangeListener, ActionListener {
	...
	private void addCalendar() {
		...
		this.vista.getDisponibilidadPanel().getCalendar()
				.setCalendar(vista.getDisponibilidadPanel().getCalendar().getCalendar());
	}

	private void addDatesToHighlight(HashMap<LocalDate, Integer> horariosFecha) {
		Map<LocalDate, Integer> fechasGreen = horariosFecha.entrySet().stream().filter(map -> map.getValue() <= 4)
		...
		Map<LocalDate, Integer> fechasRed = horariosFecha.entrySet().stream().filter(map -> map.getValue() > 10)
				.collect(Collectors.toMap(p -> p.getKey(), p -> p.getValue()));
	...
	private void loadHighlightEvaluatorGreen(Set<LocalDate> fechas) {
		...
		for (LocalDate date : fechas) {
			...
			highlightGreen.add(highlightGreen.createDate(day, month - 1, year, 0, 0, 0));
		}
	}
	...
	private void loadHighlightEvaluatorYellow(Set<LocalDate> fechas) {
		...
		for (LocalDate date : fechas) {
			...
			int month = date.getMonthValue();
			int year = date.getYear();
			highlightYellow.add(highlightYellow.createDate(day, month - 1, year, 0, 0, 0));
		}
	}
	...
	private void loadHighlightEvaluatorRed(Set<LocalDate> fechas) {
		...
		for (LocalDate date : fechas) {
			...
			int month = date.getMonthValue();
			int year = date.getYear();
			highlightRed.add(highlightRed.createDate(day, month - 1, year, 0, 0, 0));
		}
	}
	...
	private void calculateHours(HashMap<LocalDate, Integer> horariosFecha, DisponibilidadControlador dato,
			LocalDate fecha) {
	...
	private void loadTableWithHours(ArrayList<DisponibilidadControlador> disponibilidades) {
		...
		this.vista.getDisponibilidadPanel().getModelHorarios()
				.setColumnIdentifiers(this.vista.getDisponibilidadPanel().getNombreColumnasHorarios());
		...
		for (DisponibilidadControlador disponibilidad : disponibilidades) {
			Object[] fila = { this.stringDate(disponibilidad.getHoraInicio(), "HH:mm"),
					this.stringDate(disponibilidad.getHoraFin(), "HH:mm"), disponibilidad.getNombreCurso() };
		...
		this.vista.getDisponibilidadPanel().getTableHorarios().getColumnModel().getColumn(0)
				.setCellRenderer(leftRenderer);
	}

	private void paintCalendar() {
		vista.getDisponibilidadPanel().getCalendar()
				.setCalendar(vista.getDisponibilidadPanel().getCalendar().getCalendar());
	}
