...
public class CursadaABMControlador implements ActionListener {
	...
	public CursadaABMControlador(CursadaABMPanel vista, CursadaABMVistaPrincipalControlador vistaPrincipalControlador,
		...
		this.cursadas = null;
		this.usuarioLogueado = modelo.getUsuarioLogueado();
		...
		this.vista.getTextInstructor().addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				...
				try {
					listaTemporalUsuarios
							.add(usuariosLista.stream()
									.filter(UsuarioDTO -> UsuarioDTO.getUsuario().contains(
											vista.getTextInstructor().getText()) && UsuarioDTO.getIdCategoria() == 3)
									.collect(Collectors.toList()).get(0));
					System.out
							.println(usuariosLista.stream()
									.filter(UsuarioDTO -> UsuarioDTO.getUsuario().contains(
											vista.getTextInstructor().getText()) && UsuarioDTO.getIdCategoria() == 3)
									.collect(Collectors.toList()).get(0));
					listaTemporalUsuarios
	...
	private void llenarTablaAsignaciones() {
		...
		for (CursadaDTO cursadaDTO : cursadas) {
			...
			Object[] fila = { curso.getNombre(), curso.getTema(), asd(cursadaDTO.getIdEstadoCurso()),
					cursadaDTO.getFechaInicioCursada() };
	...
	private void llenarTablaCursadas() {
		...
		// this.cursadasLista = modelo.obtenerCursadasCompletas();
		for (CursadaCompletaDTO cursadaCompletaDTO : cursadasLista) {
			Object[] fila = { cursadaCompletaDTO.getIdCursada(), cursadaCompletaDTO.getIdEmpresa(),
					cursadaCompletaDTO.getIdCurso(), cursadaCompletaDTO.getIdEstadoCurso(),
					cursadaCompletaDTO.getIdAdministrativo(), cursadaCompletaDTO.getCurso(),
					...
					cursadaCompletaDTO.getEstadoCurso(), cursadaCompletaDTO.getVacantes(),
		...
		this.vista.getTblCursadas().getSelectionModel().addListSelectionListener((ListSelectionEvent event) -> {
			try {
				if (this.vista.getTblCursadas().getSelectedRow() >= 0) {
					// "idCursada", "idEmpresa", "idCurso", "idEstadoCurso",
					// "idAdministrativo","Curso",
					// "Empresa", "Ini Inscrip", "Fin Inscrip", "EstadoCurso", "Vacantes", "Ini
					// Cursada", "DiasDeClase"};

					Object idCursada = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 0);
					Object idEmpresa = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 1);
					Object idCurso = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 2);
					Object idEstadoCurso = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 3);
					Object idAdministrativo = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 4);
					Object curso = this.vista.getTblCursadas().getValueAt(this.vista.getTblCursadas().getSelectedRow(),
							5);
					Object empresa = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 6);
					Object fechaInicioInscripcion = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 7);
					Object fechaFinInscripcion = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 8);
					Object esatdoCurso = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 9);
					Object vacantes = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 10);
					Object fechaInicioCursada = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 11);
					Object diasDeClase = this.vista.getTblCursadas()
							.getValueAt(this.vista.getTblCursadas().getSelectedRow(), 12);
					...
					if (selectEstadoDeCurso(idEstadoCurso).getIdEstadoDeCurso() == 1
							&& this.modelo.getUsuarioLogueado().getIdCategoria() == 2) {
...
//		filtrarPorInstructor();
	...
	private void filtrarPorUsuario() {
		String usuario = Long.toString(this.usuarioLogueado.getIdUsuario());
    	if (this.modelo.getUsuarioLogueado().getIdCategoria() == 2) {
        	this.vista.getModeloOrdenado().setRowFilter(RowFilter.regexFilter(usuario, 4));
		} else if (this.modelo.getUsuarioLogueado().getIdCategoria() == 3) {
	    	this.vista.getModeloOrdenado().setRowFilter(RowFilter.regexFilter(usuario, 8));
		}
	}
	...
	private void filtrarPorInstructor() {
		if (this.modelo.getUsuarioLogueado().getIdCategoria() == 2) {
			this.cursadasLista = cursadasLista.stream()
					.filter(cursada -> cursada.getIdAdministrativo() == this.modelo.getUsuarioLogueado().getIdUsuario())
					.collect(Collectors.toList());
		} else if (this.modelo.getUsuarioLogueado().getIdCategoria() == 3) {
			this.cursadasLista = cursadasLista.stream()
					.filter(cursada -> cursada.getIdInstructor() == this.modelo.getUsuarioLogueado().getIdUsuario())
					.collect(Collectors.toList());
		}
	}
	...
	private boolean controlarEstadoCursada(CursadaCompletaDTO cursadaCompletaDTO) {
		...
		CursadaDTO cursadaDTO = new CursadaDTO(cursadaCompletaDTO.getIdCursada(), cursadaCompletaDTO.getIdEmpresa(),
				cursadaCompletaDTO.getIdCurso(), cursadaCompletaDTO.getIdEstadoCurso(),
				cursadaCompletaDTO.getIdAdministrativo(), cursadaCompletaDTO.getIdInstructor(),
				cursadaCompletaDTO.getFechaInicioInscripcion(), cursadaCompletaDTO.getFechaFinInscripcion(),
				cursadaCompletaDTO.getVacantes(), cursadaCompletaDTO.getFechaInicioCursada(),
				cursadaCompletaDTO.getDiasDeClase());
	...
	private CursadaDTO getCursadaDTO() {
		...
		CursadaDTO cursadaDTO = new CursadaDTO(idTemp, empresa.getIdEmpresa(), curso.getIdCurso(),
				estadoDeCurso.getIdEstadoDeCurso(), Long.parseLong(this.vista.getTextidAdministrativo().getText()),
				Long.parseLong(this.vista.getTextIdIntructor().getText()), fechaInicioInscripcion, fechaFinInscripcion,
				this.vista.getTextVacantes().getText(), fechaInicioCursada,
	...
	private String cursadaDTOToString(CursadaDTO cursadaDTO) {
		...
		String strCursadaDTO = "Curso: " + curso.getNombre() + System.lineSeparator() + "Empresa: "
				+ empresa.getNombre() + System.lineSeparator() + "Ini Insc: "
				+ stringToLocalDateFormatter(cursadaDTO.getFechaInicioInscripcion()) + System.lineSeparator()
				+ "Fin Insc: " + stringToLocalDateFormatter(cursadaDTO.getFechaFinInscripcion())
				+ System.lineSeparator() + "Ini Cursada"
				+ stringToLocalDateFormatter(cursadaDTO.getFechaInicioCursada()) + System.lineSeparator() + "Vacantes: "
				+ cursadaDTO.getVacantes() + System.lineSeparator() + "Dias de Clase: " + cursadaDTO.getDiasDeClase()
				+ System.lineSeparator() + "Estado: " + estadoCurso.getNombre();
	...
	/**
	 * @param vistaPrincipalControlador
	 *            the vistaPrincipalControlador to set
	 */
	...
	/**
	 * @param usuarioModalPanel
	 *            the usuarioModalPanel to set
	 */
	...
	/**
	 * @param usuarioModalControlador
	 *            the usuarioModalControlador to set
	 */
