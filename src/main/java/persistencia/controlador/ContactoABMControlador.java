...
public class ContactoABMControlador implements ActionListener {
	...
	private void eliminarContacto() {        
		if(hayTareaSeleccionada()){
			if(datosValidos()) {
				int eliminar = OptionPanel.confimarcion("¿Estas seguro que deseas eliminar el contacto?", "Eliminar contacto");
				if (eliminar == 0){
	...
	private void actualizarContacto() {
		if(hayTareaSeleccionada()){
			if(datosValidos()) {
				int actualizar = OptionPanel.confimarcion("¿Estas seguro que deseas cambiar los datos del contacto?", "Cambiar datos del contacto");
				if (actualizar == 0){
					CursoDTO curso = (CursoDTO) this.vista.getComboBoxCursoDeInteres().getSelectedItem();
					ContactoDTO contacto = new ContactoDTO(Long.parseLong(vista.getTxtIDContacto().getText()),
							Long.parseLong(vista.getTxtIDAlumno().getText()),
							currentContacto.getIdAdministrativo(),
							curso.getIdCurso(),
							this.vista.getTxtDescripcion().getText(),
							StringToLocalDateTime(this.vista.getTxtProximoContacto().getText(), "00:00:00"),
							currentContacto.getFechaCreacion(),
							currentContacto.getEstado());
