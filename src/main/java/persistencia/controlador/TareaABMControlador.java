...
public class TareaABMControlador implements ActionListener {
	...
	public TareaABMControlador(TareaABMPanel vista,
	...
	public void llenarTabla() {
		...
		this.vista.getTableTareas().getSelectionModel()
		...
		this.vista.getTableTareas().getColumnModel().getColumn(0)
	...
	private void clearTableTareas() {
		...
		this.vista.getModelTareas().setColumnIdentifiers(
	...
	private void loadDataRowsTbTareas() {
		for (TareaDTO tareaDTO : tareasLista) {
			Object[] fila = {
					...
					StringToLocalDateFormatter(tareaDTO.getFechaCreacion(),
							"dd/MM/yyyy"),
					StringToLocalDateFormatter(tareaDTO.getFechaCreacion(),
							"HH:mm:ss"),
					StringToLocalDateFormatter(tareaDTO.getFechaRealizar(),
							"dd/MM/yyyy"),
					StringToLocalDateFormatter(tareaDTO.getFechaRealizar(),
							"HH:mm:ss"),
					StringToLocalDateFormatter(tareaDTO.getFechaCierre(),
							"dd/MM/yyyy"),
					StringToLocalDateFormatter(tareaDTO.getFechaCierre(),
							"HH:mm:ss"), tareaDTO.getIdAlumno() };
	...
	private void ocultarColumnaTbTareas(int column) {
		this.vista.getTableTareas().getColumnModel().getColumn(column)
				.setWidth(0);
		this.vista.getTableTareas().getColumnModel().getColumn(column)
				.setMinWidth(0);
		this.vista.getTableTareas().getColumnModel().getColumn(column)
	...
	private void llenarTablaAdministrativos() {
		...
		this.panelAdministrativos.getTableAdministrativos().setSelectionModel(
				new ListSelectionModelCstm());
		this.panelAdministrativos.getTableAdministrativos().getSelectionModel()
				.addListSelectionListener((ListSelectionEvent event) -> {
					seleccionarFilaAdministrativos();
				});
		DefaultTableCellRenderer leftRenderer = new DefaultTableCellRenderer();
		leftRenderer.setHorizontalAlignment(SwingConstants.LEFT);
		this.panelAdministrativos.getTableAdministrativos().getColumnModel()
				.getColumn(0).setCellRenderer(leftRenderer);
	}
	...
	private void clearTableAdministrativos() {
		this.panelAdministrativos.getModelAdministrativos().setRowCount(0); // Para
																			// vaciar
																			// la
																			// tabla
		this.panelAdministrativos.getModelAdministrativos().setColumnCount(0);
		this.panelAdministrativos.getModelAdministrativos()
				.setColumnIdentifiers(
						this.panelAdministrativos
								.getNombreColumnasAdministrativos());
	}
	...
	private void ocultarColumnaTbAdministrativos(int column) {
		this.panelAdministrativos.getTableAdministrativos().getColumnModel()
				.getColumn(column).setWidth(0);
		this.panelAdministrativos.getTableAdministrativos().getColumnModel()
				.getColumn(column).setMinWidth(0);
		this.panelAdministrativos.getTableAdministrativos().getColumnModel()
				.getColumn(column).setMaxWidth(0);
	}
	...
	private void filtrarAdministrativos() {
		this.administrativosLista = this.administrativosLista.stream()
				.filter(usuario -> usuario.getIdCategoria() == 2)// 2 es
																	// administrativo
	...
	public void filtro() {
		...
		pendientesAndMisTareas.add(RowFilter.regexFilter(
				Long.toString(this.currentAdministrativo.getIdUsuario()), 5));

		realizadasAndMisTareas.add(RowFilter.regexFilter("Realizada", 3));
		realizadasAndMisTareas.add(RowFilter.regexFilter(
				Long.toString(this.currentAdministrativo.getIdUsuario()), 5));

		RowFilter<TableModel, Integer> filtroPendientes = RowFilter
				.andFilter(pendientesAndMisTareas);
		RowFilter<TableModel, Integer> filtroRealizadas = RowFilter
				.andFilter(realizadasAndMisTareas);

		if (this.vista.getCboxEstado().getSelectedItem().toString() == "Pendientes"
				&& this.vista.getCboxTareas().getSelectedItem().toString() == "Todas") {
			this.vista.getModeloOrdenado().setRowFilter(
					RowFilter.regexFilter("Pendiente", 3));
		} else if (this.vista.getCboxEstado().getSelectedItem().toString() == "Realizadas"
				&& this.vista.getCboxTareas().getSelectedItem().toString() == "Todas") {
			this.vista.getModeloOrdenado().setRowFilter(
					RowFilter.regexFilter("Realizada", 3));
		} else if (this.vista.getCboxEstado().getSelectedItem().toString() == "Todas"
				&& this.vista.getCboxTareas().getSelectedItem().toString() == "Todas") {
			this.vista.getModeloOrdenado().setRowFilter(null);
		} else if (this.vista.getCboxEstado().getSelectedItem().toString() == "Pendientes"
				&& this.vista.getCboxTareas().getSelectedItem().toString() == "Mis tareas") {
			...
			this.vista.getModeloOrdenado().setRowFilter(
					RowFilter.regexFilter(
							Long.toString(this.currentAdministrativo
									.getIdUsuario()), 5));
		}
	}
	...
	private String getAdministrativoString(Long idAdministrativo) {
		...
		for (UsuarioDTO usuarioDTO : this.administrativosLista) {
			if (usuarioDTO.getIdUsuario() == idAdministrativo) {
				administrativoNombre = usuarioDTO.getNombre() + " "
	...
	public void agregarTarea() {
		if (datosValidos()) {
			TareaDTO tarea = new TareaDTO(0, Long.parseLong(this.vista
					.getTxtIDResponsable().getText()),
					Long.parseLong(this.vista.getTxtIDAlumno().getText()),
					this.vista.getTxtNombre().getText(), this.vista
							.getTxtAreaDescripcion().getText(), "Pendiente",
					LocalDateTime.now(), StringToLocalDateTime(this.vista
							.getTxtFecha().getText(), "00:00:00"), null);
			this.modelo.agregarTarea(tarea);
			NotificacionDTO notificacion = new NotificacionDTO(0,
					Long.parseLong(this.vista.getTxtIDResponsable().getText()),
					2, "Se te ha asignado una tarea. Revisa la sección Tareas",
					false, LocalDateTime.now());
			this.modelo.agregarNotificacion(notificacion);
			OptionPanel.mensaje("La tarea ha sido agregada correctamente",
					"Tarea");
			generarTablas();
			clearTextInputsBox();
		}
	...
	public void modificarTarea() {
		if (hayTareaSeleccionada()) {
			if (datosValidos()) {
				int actualizar = OptionPanel
						.confimarcion(
								"Esta seguro que desea actualizar los datos de esta tarea?",
								"Actualizar tarea");
				if (actualizar == 0) {
					TareaDTO tarea = new TareaDTO(Long.parseLong(this.vista
							.getTxtID().getText()), Long.parseLong(this.vista
							.getTxtIDResponsable().getText()),
							Long.parseLong(this.vista.getTxtIDAlumno()
									.getText()), this.vista.getTxtNombre()
									.getText(), this.vista
									.getTxtAreaDescripcion().getText(),
							this.vista.getTxtEstado().getText(),
							LocalDateTime.now(), StringToLocalDateTime(
									this.vista.getTxtFecha().getText(),
									"00:00:00"), null);
					this.modelo.actualizarTarea(tarea);
					NotificacionDTO notificacion = new NotificacionDTO(
							0,
							Long.parseLong(this.vista.getTxtIDResponsable()
									.getText()),
							2,
							"Se te ha asignado una tarea. Revisa la sección Tareas",
							false, LocalDateTime.now());
					this.modelo.agregarNotificacion(notificacion);
					OptionPanel.mensaje(
	...
	public void eliminarTarea() {
		if (hayTareaSeleccionada()) {
			if (datosValidos()) {
				int eliminar = OptionPanel.confimarcion(
	...
	private void cerrarTarea() {
		if (hayTareaSeleccionada()) {
			if (datosValidos()) {
				int cerrar = OptionPanel.confimarcion(
						"Esta seguro que desea cerrar esta tarea?",
						"Cerrar tarea");
				if (cerrar == 0) {
					TareaDTO tarea = new TareaDTO(Long.parseLong(this.vista
							.getTxtID().getText()), Long.parseLong(this.vista
							...
							Long.parseLong(this.vista.getTxtIDAlumno()
									...
									.getText(), this.vista
									.getTxtAreaDescripcion().getText(),
							"Realizada", LocalDateTime.now(),
							StringToLocalDateTime(this.vista.getTxtFecha()
									.getText(), "00:00:00"),
							LocalDateTime.now());
					this.modelo.actualizarTarea(tarea);
					OptionPanel.mensaje(
							"La tarea ha sido cerrada correctamente", "Tarea");
					generarTablas();
					clearTextInputsBox();
					if (tarea.getIdTarea() > 0) {
						if (OptionPanel.confimarcion(
								"¿Desea generar un nuevo contacto?",
								"Generar Contacto") == 0) {
							...
							ctr = new TareaContactoControlador(dialog, modelo,
									this);
							ctr.setTarea(tarea);
							ctr.inicializar();
						}
	...
	private void clearTextInputsBox() {
		...
		this.vista.getTxtDetalleContactoOrigen().setText("");;
	...
	public void clickSeleccionResponsable() {
		...
		this.panelAdministrativos = this.popupAdministrativos
				.getTareaAdministrativosPanel();
	...
	private void seleccionarFila() {
		try {
			if (this.vista.getTableTareas().getSelectedRow() >= 0) {
				Object idTarea = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 0);
				Object nombre = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 1);
				Object descripcion = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 2);
				Object estado = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 3);
				Object responsable = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 4);
				Object idResponsable = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 5);
				Object fechaCreacion = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 6);
				Object horaCreacion = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 7);
				Object fechaRealizar = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 8);
				Object horaRealizar = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 9);
				Object fechaCierre = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 10);
				Object horaCierre = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 11);
				Object idAlumno = this.vista.getTableTareas().getValueAt(
						this.vista.getTableTareas().getSelectedRow(), 12);
				...
				TareaDTO tarea = new TareaDTO(
						...
						Long.parseLong(idAlumno.toString()), nombre.toString(),
						descripcion.toString(), estado.toString(),
						StringToLocalDateTime(fechaCreacion.toString(),
								horaCreacion.toString()),
						StringToLocalDateTime(fechaRealizar.toString(),
								horaRealizar.toString()),
						StringToLocalDateTime(fechaCierre.toString(),
								horaCierre.toString()));
				setCurrentTarea(tarea);
				System.out.println(this.currentTarea.getIdTarea());
	...
	private String getDetalle(ContactoCompletoDTO contacto) {
		String administrativo = "Administrativo: "
				+ contacto.getNombreAdministrativo() + " "
				+ contacto.getApellidoAdministrativo();
		String alumno = "Alumno: " + contacto.getAlumno().getNombre() + " "
				+ contacto.getAlumno().getApellido();
		String fecha = "Fecha del contacto: "
				+ StringToLocalDateFormatter(contacto.getContacto()
						.getFechaCreacion(), "dd/MM/yyyy");
		String proximoContacto = "Fecha del proximo contacto: "
				+ StringToLocalDateFormatter(contacto.getContacto()
						.getFechaContactar(), "dd/MM/yyyy");
	...
	private void seleccionarFilaAdministrativos() {
		try {
			if (this.panelAdministrativos.getTableAdministrativos()
					.getSelectedRow() >= 0) {
				Object idUsuario = this.panelAdministrativos
						.getTableAdministrativos().getValueAt(
								this.panelAdministrativos
										.getTableAdministrativos()
										.getSelectedRow(), 0);
				Object nombre = this.panelAdministrativos
						.getTableAdministrativos().getValueAt(
								this.panelAdministrativos
										.getTableAdministrativos()
										.getSelectedRow(), 3);
				Object apellido = this.panelAdministrativos
						.getTableAdministrativos().getValueAt(
								this.panelAdministrativos
										.getTableAdministrativos()
										.getSelectedRow(), 4);

				UsuarioDTO administrativoSeleccionado = new UsuarioDTO(
						Long.parseLong(idUsuario.toString()), 0,
						nombre.toString(), apellido.toString(), "", "", "", "");
				setAdministrativoSeleccionado(administrativoSeleccionado);
				System.out.println(this.administrativoSeleccionado
						.getIdUsuario()
						+ " "
						+ this.administrativoSeleccionado.getApellido());
			}
		} catch (Exception ex) {
			System.out.println("Error: " + ex.getMessage());
		}
	...
	private boolean hayTareaSeleccionada() {
		...
		if (estaVacio(tarea)) {
			OptionPanel.error("No hay ninguna tarea seleccionada",
					"Seleccionar tarea");
	...
	private boolean datosValidos() {
		...
		if (estaVacio(nombre) & estaVacio(fecha) & estaVacio(responsable)
				& estaVacio(descripcion)) {
			OptionPanel.error("Los campos estan vacios", "Campos vacios");
			camposValidos = false;
		} else if (estaVacio(nombre)) {
			OptionPanel.error(
					"El nombre esta vacio. Por favor, completar el campo",
					"Error");
			camposValidos = false;
		} else if (!validator.fechaValida(fecha)) {
			OptionPanel
					.error("Fecha incorrecta. Por favor, completar correctamente la fecha",
							"Error");
			camposValidos = false;
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == this.vista.getBtnAgregar()) {
			...
			tomarTarea();
		} else if (event.getSource() == this.vista
				.getBtnSelecionarResponsable()) {
			...
			filtrarPorTareas();
		} else if (event.getSource() == this.popupAdministrativos
				.getBtnSeleccionar()) {
		...
		if (!estaVacio(this.vista.getTxtID().getText())) {
			if (this.vista.getTxtEstado().getText().equals("Pendiente")) {
				TareaDTO tarea = new TareaDTO(Long.parseLong(this.vista
						...
						Long.parseLong(this.vista.getTxtIDAlumno().getText()),
						this.vista.getTxtNombre().getText(), this.vista
								.getTxtAreaDescripcion().getText(), this.vista
								.getTxtEstado().getText(),
						this.currentTarea.getFechaCreacion(),
						this.currentTarea.getFechaRealizar(), null);
				this.modelo.actualizarTarea(tarea);
				String administrativoResponsable = this.vista
						.getTxtResponsable().getText();
				OptionPanel.mensaje("Tomaste la tarea del administrativo: "
						+ administrativoResponsable, "Tarea");
				generarTablas();
				clearTextInputsBox();
			} else {
				System.out.println(this.vista.getTxtEstado().getText()
						.equals("Pendiente")
						+ " " + this.vista.getTxtEstado().getText());
				OptionPanel.mensaje(
						"La tarea seleccionada ya ha sido realizada", "Tarea");
			}
		} else {
	...
	private void seleccionarResponsable() {
		if (this.administrativoSeleccionado != null) {
			...
			String responsable = Long.toString(this.administrativoSeleccionado
					.getIdUsuario());
			System.out.println(responsable);
			this.vista.getTxtIDResponsable().setText(responsable);
			this.vista.getTxtResponsable().setText(
					this.administrativoSeleccionado.getNombre() + " "
							+ this.administrativoSeleccionado.getApellido());
